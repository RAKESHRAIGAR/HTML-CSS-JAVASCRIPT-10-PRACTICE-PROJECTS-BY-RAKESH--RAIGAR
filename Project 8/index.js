...
const renderTransactions = () => {
  const transactionContaiinerEl = document.querySelector(".transactions");
  ...
  let expense = 0;
  let net = 0;
  transactionContaiinerEl.innerHTML = "";
  transactions.forEach((transaction) => {
    const { id, amount, text, type } = transaction;
    const isCredit = type == "credit" ? true : false;
    const singn = isCredit ? "+" : "-";
  ...
  <div class="status ${isCredit ? "credit" : "debit"}">${
    ...
    transactionContaiinerEl.insertAdjacentHTML("afterbegin", transactionEl);
  });
...
const addTransaction = (e) => {
  ...
  const transaction = {
    id:isUpdate ?   Math.floor(Math.random() * 1000),
    text: text,
    amount: +amount,
